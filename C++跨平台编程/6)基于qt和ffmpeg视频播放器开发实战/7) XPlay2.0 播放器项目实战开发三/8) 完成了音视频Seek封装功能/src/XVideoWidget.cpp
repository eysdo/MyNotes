...
void XVideoWidget::Repaint(AVFrame *frame)
{
	...
	if (_width == frame->linesize[0]) // 无需对齐
	{
	...
	else // 行对齐问题
	{
		for (int i = 0; i < _height; i++)  // Y
			memcpy(_datas[0] + _width*i, frame->data[0] + frame->linesize[0]*i, _width);
		for (int i = 0; i < _height/2; i++)  // U
			memcpy(_datas[1] + _width/2*i, frame->data[1] + frame->linesize[1] * i, _width);
		for (int i = 0; i < _height/2; i++)  // V
			memcpy(_datas[2] + _width/2*i, frame->data[2] + frame->linesize[2] * i, _width);
	}
